...
local Config = {
    ...
    NpcEspHealth = true,
    NpcEspDistance = true,
    NpcEspColor = Color3.fromRGB(255, 165, 0),
...
local function FindRootPart(char)
    ...
    if root then return root end
    root = char:FindFirstChild("Chest")
    if root then return root end
    root = char:FindFirstChild("Pelvis")
    if root then return root end
    -- Last resort: find largest BasePart
    local largest = nil
    local largestSize = 0
...
local function FindHeadPart(char)
    ...
    if head then return head end
    head = char:FindFirstChild("Face")
    if head and head:IsA("BasePart") then return head end
    head = char:FindFirstChild("FaceHead")
    if head and head:IsA("BasePart") then return head end
    head = char:FindFirstChild("Skull")
    if head and head:IsA("BasePart") then return head end
    head = char:FindFirstChild("Helmet")
    if head and head:IsA("BasePart") then return head end
    -- Find the highest BasePart in the character
    local highest = nil
...
local function FindHitboxPart(char)
    ...
    if part and part:IsA("BasePart") then return part end
    -- Fallback: find any large BasePart
        ...
        for _, obj in ipairs(container:GetChildren()) do
            if obj:IsA("Model") then
                ...
                if hum and hum.Health > 0 then
                    ...
                    if obj ~= localChar then
                        -- Skip if it's another player's character
            ...
            elseif obj:IsA("Folder") or obj:IsA("Workspace") then
                ScanContainer(obj)
            end
        end
    end

    -- Scan common NPC locations
...
local function GetClosestTarget()
    ...
    -- Check NPCs (zombies, enemies, etc.)
    for _, npc in ipairs(CachedNPCs) do
        if npc.Parent and IsModelAlive(npc) then
            local part = npc:FindFirstChild(Config.AimbotPart)
            if not part or not part:IsA("BasePart") then part = FindHeadPart(npc) end
            if part then
                local screenPos, onScreen = WorldToScreen(part.Position)
                if onScreen then
                    local mousePos = UserInputService:GetMouseLocation()
...
local function CreateNPCESP(npc)
    ...
    NpcESPObjects[npc] = objects
end

local function RemoveNPCESP(npc)
    if not NpcESPObjects[npc] then return end
    for _, obj in pairs(NpcESPObjects[npc]) do
        pcall(function() obj:Remove() end)
    end
...
local function UpdateNPCESP()
    -- Refresh NPC cache every 2 seconds
    local now = tick()
    if now - LastNPCScan > 2 then
        -- Remove ESP for NPCs that no longer exist
        for npc, objects in pairs(NpcESPObjects) do
            if not npc or not npc.Parent or not IsModelAlive(npc) then
                RemoveNPCESP(npc)
            end
        end
        CachedNPCs = ScanForNPCs()
        LastNPCScan = now
    end

    if not Config.NpcEspEnabled then
        for npc, objects in pairs(NpcESPObjects) do
            for _, obj in pairs(objects) do
                if obj.Visible ~= nil then obj.Visible = false end
            end
    ...
    for _, npc in ipairs(CachedNPCs) do
        ...
        local objects = NpcESPObjects[npc]
        local rootPart = FindRootPart(npc)
        local head = FindHeadPart(npc)
        ...
        local espColor = Config.NpcEspColor

        -- Box
        if Config.NpcEspBox then
            local boxHeight = math.abs(headPos.Y - legPos.Y)
            local boxWidth = boxHeight * 0.5
            objects.Box.Size = Vector2.new(boxWidth, boxHeight)
            objects.Box.Position = Vector2.new(rootPos.X - boxWidth / 2, headPos.Y)
        ...
        -- Name
        objects.Name.Position = Vector2.new(rootPos.X, headPos.Y - 16)
        objects.Name.Text = npc.Name
        objects.Name.Color = espColor
        objects.Name.Visible = true

        -- Health
        if Config.NpcEspHealth and humanoid then
...
local function UpdateHitbox()
    ...
    for i, npc in ipairs(CachedNPCs) do
        if npc and npc.Parent then
            ApplyHitboxToCharacter(npc, npc)
        end
    end
end
...
CreateToggle(espTab, "NPC ESP", Config.NpcEspEnabled, function(v) Config.NpcEspEnabled = v end, 9)
...
-- Hitbox Tab
local hitboxTab = CreateTab("Hitbox", 5)
...
-- Player list tab
local playerTab = CreateTab("Players", 6)
